...
async function accrocherUtilisateur(userId: string, username: string) {
  ...
  verboseLog(
    `🔍 Initial voice state - User: ${
      userVoiceState?.channelId || "null"
...
async function ancrerUtilisateur(userId: string, username: string) {
  ...
  verboseLog(
    `🔍 Initial voice state (anchoring) - User: ${
      userVoiceState?.channelId || "null"
    }, You: ${currentVoiceState?.channelId || "null"}`
  );
  ...
  if (!userVoiceState?.channelId || !currentVoiceState?.channelId) {
    ...
    verboseLog(
      `🔍 Voice state after wait (anchoring) - User: ${
        userVoiceState?.channelId || "null"
      }, You: ${currentVoiceState?.channelId || "null"}`
    );
  }
...
function startAnchorMonitoring() {
  ...
  anchorMonitoringInterval = setInterval(() => {
    ...
    if (!myVoiceState?.channelId || !anchoredUserVoiceState?.channelId) {
      verboseLog(
        `🔍 Anchor monitoring: One of the users is not in a voice channel - You: ${
          myVoiceState?.channelId || "null"
...
const UserContextMenuPatch: NavContextMenuPatchCallback = (
  ...
  if (!settings.store.enabled || !user) {
    ...
    verboseLog(
      `❌ Plugin disabled or user missing - enabled: ${
  ...
  if (!currentUser || user.id === currentUser.id) {
    verboseLog(
      `❌ Current user missing or same user - currentUser: ${!!currentUser}, sameUser: ${
    ...
    async VOICE_STATE_UPDATES({ voiceStates }: { voiceStates: VoiceState[] }) {
        ...
        for (const voiceState of voiceStates) {
          ...
          if (
            ...
            if (channelId !== anchoredUserVoiceState.channelId) {
              log(
                `⚠️ You have been moved, automatic return to ${
                  anchoredUserInfo!.username
                }'s channel`
              );
      ...
      for (const voiceState of voiceStates) {
        ...
        if (
          ...
          verboseLog(
            `🔄 Channel change detected for ${
              accrochedUserInfo!.username
