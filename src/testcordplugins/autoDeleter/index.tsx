...
const settings = definePluginSettings({
  ...
  deleteKeywords: {
    type: OptionType.STRING,
    description:
      "Mots-clés pour suppression immédiate (séparés par des virgules)",
    default: "",
  },
  maxMessageLength: {
    type: OptionType.NUMBER,
  ...
  useAntiLogDeletion: {
    type: OptionType.BOOLEAN,
    description:
      "Utiliser la suppression AntiLog (masque les messages des plugins MessageLogger)",
    default: true,
  },
  blockMessage: {
    type: OptionType.STRING,
    description:
      "Texte à afficher à la place du message supprimé (pour AntiLog)",
    default: "x",
  },
  deleteInterval: {
    type: OptionType.NUMBER,
    description:
      "Délai entre la suppression de l'ancien et du nouveau message (ms) - pour AntiLog",
    default: 200,
  },
  rateLimitHandling: {
    type: OptionType.BOOLEAN,
  ...
  circuitBreakerThreshold: {
    type: OptionType.NUMBER,
    description:
      "Seuil pour ouvrir le circuit breaker (rate limits consécutifs)",
    default: 5,
    min: 3,
    max: 20,
  },
  ...
  maxMessagesPerHour: {
    type: OptionType.NUMBER,
    description:
      "Nombre maximum de messages à supprimer par heure (0 = illimité)",
    default: 0,
    min: 0,
    max: 1000,
  },
...
export default definePlugin({
  ...
  async start() {
    ...
    this.log(
      `Restauration terminée: ${this.stats.restoredFromStorage} messages restaurés`
    );
  },
    ...
    try {
      ...
      for (const savedMsg of savedMessages) {
        ...
        if (timeUntilDeletion <= 0) {
          this.debug(
            `Message ${savedMsg.id} aurait dû être supprimé, suppression immédiate`
          );
  ...
  scheduleMessageDeletion(message: any, customDelay?: number) {
    ...
    this.debug(
      `Programmation suppression message ${
        message.id
      } dans ${delay}ms (${new Date(
        scheduledTime
  ...
  scheduleMessageDeletionFromRestore(savedMsg: TrackedMessage, delay: number) {
    this.debug(
      `Reprogrammation suppression message ${savedMsg.id} dans ${delay}ms`
    );
    ...
    try {
      ...
      if (
        ...
        if (timeSinceLastRateLimit < this.rateLimitInfo.retryAfter * 1000) {
          this.debug(
            `Rate limit actif, ajout à la queue de retry: ${messageId}`
          );
  ...
  handleRateLimit(
    ...
    if (this.rateLimitInfo.consecutiveRateLimits >= threshold) {
      ...
      this.log(
        `Circuit breaker ouvert pour ${settings.store.circuitBreakerDuration} minutes`
      );
    }
  ...
  exportStats() {
    ...
    link.href = url;
    link.download = `autodeleter-stats-${
