...
async function copyProfileFromInput(input: string) {
    ...
    if (targetUser.globalName) {
        payload.pendingGlobalName = targetUser.globalName;
    }

    if (profile?.pronouns) {
        payload.pendingPronouns = profile.pronouns;
    }
    ...
    if (nitro) {
        if (profile?.themeColors) payload.pendingThemeColors = profile.themeColors;
        if (profile?.accentColor != null) payload.pendingAccentColor = profile.accentColor;
    }
    ...
    if (primaryGuild?.identityGuildId && primaryGuild.identityEnabled !== false) {
        ...
        if (sharedGuild) {
            payload.pendingPrimaryGuildId = primaryGuild.identityGuildId;
        }
    }

    if (Object.keys(payload).length === 0 && bioToSet == null) {
        showToast("Nothing to copy from that profile.", Toasts.Type.MESSAGE);
        return;
    }

    if (Object.keys(payload).length > 0) setPendingChanges(payload);
    if (bioToSet != null) setPendingChanges({ pendingBio: bioToSet });
    ...
    showToast(`Copied profile from ${targetUser.username}. Click Save to apply.`, Toasts.Type.SUCCESS);
}
...
export default definePlugin({
    ...
    description: "Adds a button to the profile editor that fills your profile with another user's info (avatar, banner, bio, status, clan tag).",
