...
function isUserWhitelisted(userId: string): boolean {
  ...
  verboseLog(
    `🔍 Whitelist check for user ${userId}: ${
      isWhitelisted ? "AUTHORIZED" : "NOT AUTHORIZED"
    }`
  );
...
function wasRecentlyAdded(channel: any, currentUserId: string): boolean {
  ...
  verboseLog(
    `🔍 Recent addition check: ${
      wasAdded ? "ADDED BY SOMEONE ELSE" : "CREATED BY YOU"
    } (Owner: ${channel.ownerId})`
  );
...
- Current user: ${currentUserId}`);
      ...
      log(
        `🎯 NEW DM GROUP DETECTED: "${channel.name || "Unnamed"}" (${
          channel.id
        })`
      );
